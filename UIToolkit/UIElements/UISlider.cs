...
public class UISlider : UITouchableSprite
{
	...
	public static UISlider create( UIToolkit manager, string knobFilename, string trackFilename, int trackxPos, int trackyPos, UISliderLayout layout, int depth = 2, bool knobInFront = true )
	{
		...
		return new UISlider( manager, knobFilename, trackFrame, depth, trackTI.uvRect, knob, layout );
	}
	

	public UISlider( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, UISprite sliderKnob, UISliderLayout layout ):base( frame, depth, uvFrame )
	{
		this.layout = layout;
	...
	// Current value of the slider.  Will always be between 0 and 1.
	public float value
	{
		get { return _value; }
		set
