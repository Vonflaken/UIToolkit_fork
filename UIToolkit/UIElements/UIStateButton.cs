...
public class UIStateButton : UIButton
{
	...
	public static UIStateButton create( UIToolkit manager, string[] filenames, string[] highlightedFilenames, int xPos, int yPos, int depth )
	{
		...
		var button = new UIStateButton( manager, filenames[0], frame, depth, firstNormalTI.uvRect, firstHighlightedTI.uvRect );
	...
	public UIStateButton( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, UIUVRect highlightedUVframe ):base( manager, filename, frame, depth, uvFrame, highlightedUVframe )
	{
	}
