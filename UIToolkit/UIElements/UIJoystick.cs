...
public class UIJoystick : UITouchableSprite
{
	...
	public static UIJoystick create( UIToolkit manager, string joystickFilename, Rect hitAreaFrame, float xPos, float yPos )
	{
		...
		return new UIJoystick( manager, joystickFilename, hitAreaFrame, 1, joystick, xPos, yPos );
	}

	
	public UIJoystick( UIToolkit manager, string filename, Rect frame, int depth, UISprite joystickSprite, float xPos, float yPos ):base( frame, depth, UIUVRect.zero )
	{
