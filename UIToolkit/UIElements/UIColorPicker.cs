...
public class UIColorPicker : UITouchableSprite
{
	...
	public static UIColorPicker create( UIToolkit manager, string filename, int xPos, int yPos, int depth )
	{
		...
		// create the button
		return new UIColorPicker( manager, filename, frame, depth, normalTI.uvRect, textureCoords );
	}


	public UIColorPicker( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, Vector2 textureCoords ):base( frame, depth, uvFrame )
	{
	...
	private Vector2 getTouchTextureCoords( Vector2 touchPos )
	{
		...
		float localxPos = touchPos.x - position.x;
		float normLocalxPos = localxPos / width;
		float originalLocalxPos = normLocalxPos * originalWidth;
		float xPos = textureCoords.x + originalLocalxPos;
		...
		float localyPos = touchPos.y - ( -1 * position.y );
		float normLocalyPos = localyPos / height;
		float originalLocalyPos = normLocalyPos * originalHeight;
		float yPos = (manager.material.mainTexture.height - textureCoords.y) - originalLocalyPos;
