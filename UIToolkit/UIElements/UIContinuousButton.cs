...
public class UIContinuousButton : UIButton
{
	...
	public static new UIContinuousButton create( UIToolkit manager, string filename, string highlightedFilename, int xPos, int yPos, int depth )
	{
		...
		// create the button
		return new UIContinuousButton( manager, filename, frame, depth, normalTI.uvRect, highlightedTI.uvRect );
	}


	public UIContinuousButton( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, UIUVRect highlightedUVframe ):base( manager, filename, frame, depth, uvFrame, highlightedUVframe )
	{
	}
