...
public class UIZoomButton : UIButton
{
	...
	public static new UIZoomButton create( UIToolkit manager, string filename, string highlightedFilename, int xPos, int yPos, int depth )
	{
		...
		// create the button
		return new UIZoomButton( manager, filename, frame, depth, normalTI.uvRect, highlightedTI.uvRect );
	}


	public UIZoomButton( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, UIUVRect highlightedUVframe ):base( manager, filename, frame, depth, uvFrame, highlightedUVframe )
	{
		centerize();
