...
public class UIKnob : UITouchableSprite
{
	...
	public static UIKnob create( UIToolkit manager, string filename, string highlightedFilename, int xPos, int yPos, int depth )
	{
		...
		// create the button
		return new UIKnob( manager, filename, frame, depth, normalTI.uvRect, highlightedTI.uvRect );
	}

	
	public UIKnob( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, UIUVRect highlightedUVframe ):base( frame, depth, uvFrame, true )
	{
