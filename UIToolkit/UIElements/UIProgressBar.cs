...
public class UIProgressBar : UISprite
{
	...
	public static UIProgressBar create( UIToolkit manager, string barFilename, int xPos, int yPos, bool rightToLeft, int depth )
	{
		...
		var progressBar = new UIProgressBar( manager, barFilename, frame, depth, textureInfo.uvRect, rightToLeft );
		
		return progressBar;
	}
	
	
	public UIProgressBar( UIToolkit manager, string filename, Rect frame, int depth, UIUVRect uvFrame, bool rightToLeft ):base( frame, depth, uvFrame )
	{
