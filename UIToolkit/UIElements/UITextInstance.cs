...
public class UITextInstance : UIObject, IPositionable, ITouchable, IComparable
{
	public event Action<UITextInstance> onTouchUp;
	public event Action<UITextInstance,bool> onDisabled;
	...
	protected bool touchFrameBuilded = false; // HACK: Indicates if the touchFrame needs to be recalculated for the first time
	...
	public virtual bool disabled
	{
		...
		set
		{
			if( _disabled != value )
			{
				...
				if ( onDisabled != null )
					onDisabled( this, value );
				
				// if we have something to show use it
			}
		}
	}
	
	public UIText parentText
	{
	...
	/// </summary>
	public UITextInstance( UIText parentText, string text, float xPos, float yPos, bool touchable, float scale, int depth, Color color ) : this( parentText, text, xPos, yPos, touchable, scale, depth, new Color[] { color }, parentText.alignMode, parentText.verticalAlignMode )
	{}
	...
	/// </summary>
	public UITextInstance( UIText parentText, string text, float xPos, float yPos, bool touchable, float scale, int depth, Color[] colors, UITextAlignMode alignMode, UITextVerticalAlignMode verticalAlignMode ) : base()
	{
	...
	public Rect touchFrame
	{
		get
		{
			...
			if( touchFrameIsDirty )
			{
				...
				if( _alignMode == UITextAlignMode.Center && _verticalAlignMode == UITextVerticalAlignMode.Middle )
				{
			...
			// Return normal touch frame
			return _normalTouchFrame;
		}
	}
	...
	{
		if ( !touchFrameBuilded )
		{
			touchFrameIsDirty = true;
			touchFrameBuilded = true;
		}
	...
	{
		// Detach from manager
		manager.removeFromTouchables( this );
	...
	public virtual void onKeyboardEntry( string inputString, string deltaInputString )
	{
