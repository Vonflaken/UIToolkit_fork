...
public class UIGridLayout : UIAbstractContainer
{
	...
	public override void addChild (params UIObject[] children)
	{
		...
		{
			var toCol = Mathf.FloorToInt(i / rows);
			var toRow = i - toCol * rows;
			...
			if ( children[i] is UISprite )
			{
				((UISprite) children[i]).beginUpdates();
					children[i].anchorInfo = anchorInfo;
					children[i].refreshPosition();
				((UISprite) children[i]).endUpdates();
			}
			if ( children[i] is UITextInstance )
			{
				children[i].anchorInfo = anchorInfo;
				children[i].refreshPosition();
			}
	...
	UIAnchorInfo GetCellAnchorFor(int column, int row)
	{
		...
		if ( _contentWidth != 0f && _contentHeight != 0f )
		{
			cellWidth  = _contentWidth / 2f + spacing; // HACK: _contentWidth is double sized since It called twice to base.addChild(...)
			cellHeight = _contentHeight / 2f + spacing; // same HACK as above
		}
		else
		{
		...
		cellAnchor.OffsetX = cellWidth * column + _edgeInsets.left;
		cellAnchor.OffsetY = cellHeight * row + _edgeInsets.right;
