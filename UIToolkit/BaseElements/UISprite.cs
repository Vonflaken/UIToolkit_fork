...
public class UISprite : UIObject, IPositionable
{
	...
	{
		UIUVRect tempUVFrame = manager.uvRectForFilename( filename );

		if ( this is UITouchableSprite ) // HACK: keeping equal hoveredUVFrame and uvFrame
		{
			UITouchableSprite touchSpr = (UITouchableSprite) this;
			if ( touchSpr.hoveredUVframe == uvFrame )
				touchSpr.hoveredUVframe = tempUVFrame;
		}
