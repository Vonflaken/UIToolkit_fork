...
public class UISpriteManager : MonoBehaviour
{
	...
	protected UISprite[] _sprites = new UISprite[0];    // Array of all sprites (the offset of the vertices corresponding to each sprite should be found simply by taking the sprite's index * 4 (4 verts per sprite).
	...
	public UISprite addSprite( string name, int xPos, int yPos, int depth, bool gameObjectOriginInCenter )
	{
		...
		return this.addSprite( name, positionRect, textureInfo.uvRect, depth, gameObjectOriginInCenter );
	}
	...
	/// </summary>
	private UISprite addSprite( string filename, Rect frame, UIUVRect uvFrame, int depth, bool gameObjectOriginInCenter )
	{
		// Create and initialize the new sprite
		UISprite newSprite = new UISprite( frame, depth, uvFrame, gameObjectOriginInCenter );
		addSprite( newSprite, filename );
	...
	/// </summary>
	public void addSprite(UISprite sprite, string filename)
	{
	...
	/// <summary>
	/// Gets the atlas filename.
	/// </summary>
	/// <returns>The atlas filename.</returns>
	public string GetAtlasCgiFilename()
	{
		return texturePackerConfigName;
	}
