...
public abstract class UIAbstractContainer : UIObject, IPositionable
{
	...
	public virtual bool hidden
	{
		...
		set
		{
			...
			foreach( var child in _children )
			{
				...
				else if ( child is UITextInstance )
					((UITextInstance) child).hidden = value;
				else if ( child is UIAbstractContainer )
					((UIAbstractContainer) child).hidden = value;
			}
		}
	}
	...
	/// <summary>
	/// Gets the <see cref="UIAbstractContainer"/> with the specified pos.
	/// </summary>
	/// <param name="pos">Position.</param>
	public UIObject this[int pos]
	{
		get
		{
	...
	public int getElementIndex( UIObject element )
	{
	...
	public void removeChild( UIObject child, bool removeFromManager )
	{
		...
		{			
			if ( child is UITouchableSprite )
				((UITouchableSprite) child).manager.removeFromTouchables( (UITouchableSprite) child );
			else if ( child is UISprite )
				((UISprite) child).manager.removeElement( (UISprite) child );
		
			if ( child is UISprite )
				((UISprite) child).destroy();
		}
	}


	/// <summary>
	/// Removes a child from the container.  If it is removed from
	/// it's manager it is no longer in existance so be sure to null out any references to it.
	/// </summary>
	...
	protected virtual void layoutChildren()
	{
		...
		else if( _layoutType == UIAbstractContainer.UILayoutType.AbsoluteLayout )
		{
			foreach( var item in _children )
			{
				bool hidden = false;
				if ( item is UISprite )
					hidden = ((UISprite) item).hidden;
				if ( item is UITextInstance )
					hidden = ((UITextInstance) item).hidden;
	...
	public virtual void matchSizeToContentSize()
	{
