...
public abstract class UITouchableSprite : UISprite, ITouchable, IComparable
{
	...
	public delegate void UITouchableSpriteChanged( UITouchableSprite sender, bool state );
	public event UITouchableSpriteChanged onDisabled; // event for when disabled is setted
	...
	// set hoveredUVFrame to trigger hover handlers
	// whole uvframe sprite area to check hovered
	public void SetHoveredCheck( bool active )
	{
		if ( active )
		{
			if ( hoveredUVframe.Equals( UIUVRect.zero ) )
				hoveredUVframe = uvFrame; // still not setted
		}
		else
		{
			hoveredUVframe = UIUVRect.zero;
		}
	}
	...
	public virtual bool hoveredOver
	{
		...
		set
		{
			...
			if( _hoveredOver != value )
			{
				...
				{
					// if we have a hoveredUVframe use it
					if( value && !hoveredUVframe.Equals( uvFrame ) )
	...
	// hover handler.  Subclasses should override these to get their specific behaviour
	public virtual void onHovered() {}


	// out hover handler.  Subclasses should override these to get their specific behaviour
	public virtual void onOutHovered() {}


	public virtual void setDisabledSpriteImage( string filename )
	{
		disabledUVframe = manager.uvRectForFilename( filename );
	}


	public virtual void setHoveredSpriteImage( string filename )
	{
		hoveredUVframe = manager.uvRectForFilename( filename );
	}
